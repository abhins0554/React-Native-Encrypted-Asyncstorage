This package is dependent on the @react-native-async-storage/async-storage and react-native-crypto-js.

It is developed to use the asyncstorage for storing user credential & other sensitive data.

Supported platforms
    -iOS
    -Android

Usage for set encrypting data :- 
**Setting Data To Asyncstorage**
---import {Set_Encrypted_AsyncStorage} from 'react-native-encrypted-asyncstorage';

---await Set_Encrypted_AsyncStorage(type,key,data)

above line just take **type** either 'text'||'object' 
next take **key** sets a string value for given key
& last **data** you want to store either in format of object or text as specified during passing of **type**
...
await Get_Encrypted_AsyncStorage(type,key).then(reponse=>{
